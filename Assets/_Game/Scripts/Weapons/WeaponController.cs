...
namespace DLS.Weapons
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                MuzzleWorldVelocity = (WeaponMuzzle.position - m_LastMuzzlePosition) /UnityEngine.Time.deltaTime;
                m_LastMuzzlePosition = WeaponMuzzle.position;
            }
        }
        ...
        void UpdateAmmo()
        {
            if (AutomaticReload && m_LastTimeShot + AmmoReloadDelay < UnityEngine.Time.time && m_CurrentAmmo < MaxAmmo && !IsCharging)
            {
                // reloads weapon over time
                m_CurrentAmmo += AmmoReloadRate * UnityEngine.Time.deltaTime;
        ...
        void UpdateCharge()
        {
            if (IsCharging)
            {
                if (CurrentCharge < 1f)
                {
                    ...
                    {
                        chargeAdded = (1f / MaxChargeDuration) * UnityEngine.Time.deltaTime;
                    }
        ...
        public void UseAmmo(float amount)
        {
            ...
            m_LastTimeShot = UnityEngine.Time.time;
        }
        ...
        {
            if (m_CurrentAmmo >= 1f
                && m_LastTimeShot + DelayBetweenShots < UnityEngine.Time.time)
            {
                HandleShoot();
                m_CurrentAmmo -= 1f;
        ...
        bool TryBeginCharge()
        {
                ...
                && m_LastTimeShot + DelayBetweenShots < UnityEngine.Time.time)
            {
                ...
                LastChargeTriggerTimestamp = UnityEngine.Time.time;
                IsCharging = true;
