...
namespace DLS.Weapons
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        {
            MessageSystem.MessageManager.UnregisterForChannel<ProjectileShootMessage>(MessageChannels.Weapons, ProjectileShootMessageHandler);
        }

        private void ProjectileShootMessageHandler(MessageSystem.IMessageEnvelope message)
        {
            if(!message.Message<ProjectileShootMessage>().HasValue) return;
            var data = message.Message<ProjectileShootMessage>().GetValueOrDefault();
            if (data.Projectile == m_ProjectileBase)
            {
                OnShoot();
            }
        }
        ...
        {
            m_ShootTime = UnityEngine.Time.time;
            m_LastRootPosition = Root.position;
            m_Velocity = transform.forward * Speed;
            m_IgnoredColliders = new List<Collider>();
            transform.position += m_ProjectileBase.InheritedMuzzleVelocity * UnityEngine.Time.deltaTime;
        ...
        {
            // Move
            transform.position += m_Velocity * UnityEngine.Time.deltaTime;
            ...
            if (GravityDownAcceleration > 0)
            {
                ...
                m_Velocity += Vector3.down * (GravityDownAcceleration * UnityEngine.Time.deltaTime);
            }
        ...
        bool IsHitValid(RaycastHit hit)
        {
            ...
            if (hit.collider.isTrigger && hit.collider.GetComponent<IDamagable>() == null)
            {
                return false;
            }
