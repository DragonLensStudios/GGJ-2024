...
namespace AI
{
    ...
    public class EnemyMobile : MonoBehaviour
    {
        ...
        {
            MessageSystem.MessageManager.RegisterForChannel<AttackMessage>(MessageChannels.Combat,AttackMessageHandler);
        ...
        {
            MessageSystem.MessageManager.UnregisterForChannel<AttackMessage>(MessageChannels.Combat,AttackMessageHandler);
        ...
        private void HealthChangedMessageHandler(MessageSystem.IMessageEnvelope message)
        {
            ...
            if(data.Target != m_EnemyController.gameObject) return;
        ...
        private void AttackMessageHandler(MessageSystem.IMessageEnvelope message)
        {
            if(!message.Message<AttackMessage>().HasValue) return;
            var data = message.Message<AttackMessage>().GetValueOrDefault();
            if(data.Attacker != m_EnemyController.gameObject) return;
            OnAttack();
        }
