...
namespace Unity.FPS.Addons
{
    public class WeaponBarrelRecoil : MonoBehaviour
    {
        ...
        void StartRecoilAnimation()
        {
            ...
            m_StartTimestamp = UnityEngine.Time.deltaTime;
        }

        void Update()
        {
            if (m_StartTimestamp > 0f && m_StartTimestamp + Duration > UnityEngine.Time.time && Duration > 0f)
            {
                float ratio = 1f - ((m_StartTimestamp + Duration) - UnityEngine.Time.deltaTime) / Duration;
