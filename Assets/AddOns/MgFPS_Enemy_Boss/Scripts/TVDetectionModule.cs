...
namespace DLS.AddOns.MgFPS_Enemy_Boss.Scripts
{
    public class TVDetectionModule : DetectionModule
    {
        ...
        public override void OnDetect()
        {
            ...
            m_TimeStartSeeingTarget = UnityEngine.Time.time;
        ...
        public override void OnDamaged(GameObject damageSource)
        {
            ...
            m_TimeLastBeenHurt = UnityEngine.Time.time;
        ...
        private void Update()
        {
            switch (m_CurrentState)
            {
                case State.Idle:
                    break;
                case State.Detect:
                    if ((UnityEngine.Time.deltaTime - m_TimeStartSeeingTarget) > m_DetectStateDuration)
                    {
                ...
                case State.Attack:
                    break;
                case State.Hurt:
                    if ((UnityEngine.Time.deltaTime - m_TimeLastBeenHurt) > m_HurtStateDuration)
                    {
                        m_CurrentState = State.Attack;
                        SetScreenTextures(m_AttackTexture, m_AttackEmissionTexture);
                    }
